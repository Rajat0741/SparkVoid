...
import { findSharedConversation, findMessagesByConversationId } from "@/lib/db/queries";
...
export default async function SharedConversationPage({ params }: PageProps) {
  ...
  const conversation = await findSharedConversation(id);
  ...
  const [rawMessages, session] = await Promise.all([
    findMessagesByConversationId(id),
